...
let flatmap_sized f sizes blank xs =
...
let fm_s_helper f sizes xs =
    let (_, result) = flatmap_sized f sizes 0 xs
    in result
    


let sort_by_nybble (k: u8)(xs: []u64): []u64 =
    let lgth = length xs
    ...
    let grouped_xs = pad_xs
                    ...
                    --|> map (sort_few)  <--- Why doesn't this work?
                    |> map (merge_sort (<))
                    |> trace
                    |> break
    ...
    let indices = fm_s_helper f_i (map i32.u64 gramz) tuples |> trace |> break
